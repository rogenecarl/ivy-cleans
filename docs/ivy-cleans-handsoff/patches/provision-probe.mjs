...
if (!pb.k || !pb.s) bad("PORKBUN_API_KEY / PORKBUN_SECRET_KEY not set");
  ...
  try {
    ...
    r.status === "SUCCESS" ? ok(`credentials valid (your IP ${r.yourIp})`) : bad(`ping: ${r.message}`);
    ...
    c.status === "SUCCESS"
      ? ok(`checkDomain ${testDomain}: avail=${resp.avail} price=$${resp.price}`)
      : bad(`checkDomain: ${c.message}`);
    ...
    if (l.status === "SUCCESS") {
      ...
      n > 0
        ? ok(`account owns ${n} domain(s) — API registration is unlocked`)
        : bad("account owns 0 domains — Porkbun requires ONE manual registration before API registration works");
...
if (!vc.t || !vc.p) bad("VERCEL_TOKEN / VERCEL_PROJECT_ID not set");
  ...
  try {
    ...
    me.user ? ok(`token valid (${me.user.username ?? me.user.email})`) : bad(`token: ${me.error?.message}`);

    const pr = await fetch(`https://api.vercel.com/v9/projects/${vc.p}${q}`, { headers: H }).then((x) => x.json());
    pr.id ? ok(`project ${pr.name} (${pr.id})`) : bad(`project: ${pr.error?.message}`);
    ...
    if (team) {
      ...
      plan === "pro" || plan === "enterprise"
        ? ok(`team plan: ${plan}`)
        : bad(`team plan: ${plan} — Hobby caps at 50 domains per project; Pro is required`);
    }
    ...
    cfg.recommendedIPv4
      ...
      : bad(`config: ${cfg.error?.message ?? "no recommendations returned"}`);
...
if (!vc.t || !vc.gc) bad("GLOBAL_CONFIG_ID not set");
  ...
  try {
    ...
    m.id ? ok(`store "${m.slug}" — ${m.itemCount} item(s), ${m.sizeInBytes} bytes`) : bad(`store: ${m.error?.message}`);
