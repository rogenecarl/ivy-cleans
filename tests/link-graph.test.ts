...
describe.each(cities.map((c) => [c.city, c] as const))('%s', (_name, c) => {
  ...
  it('every area page is linked from a service page' + (hasNeighbours ? ' and from another area' : ''), () => {
    for (const page of areas) {
      const from = [...inboundOf.get(page)!]
      expect(from.some((p) => p.startsWith('/services/')), `${page} <- services`).toBe(true)
      if (hasNeighbours) expect(from.some((p) => areas.includes(p)), `${page} <- areas`).toBe(true)
    }
  })
